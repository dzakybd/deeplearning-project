...
def chord_to_notewise(long_string, sample_freq):
    ...
    while i < len(translated_list):
        ...
        if translated_list[i] == 'wait':
            while wait_count <= sample_freq * 2 and i + wait_count < len(translated_list) and translated_list[
                i + wait_count] == 'wait':
                wait_count += 1
...
def translate_piece(fname, composer, chamber, sample_freqs, note_ranges, note_offsets, CHORDWISE_PATH, NOTEWISE_PATH,
                    replace):
    ...
    if not replace:
        ...
        if exists:
            print(
                "Skipping file: Output files already exist. Use --replace to override this and retranslate everything.")
            return
    ...
    for sample_freq in sample_freqs:
        for note_range in note_ranges:
            ...
            score_string_arr = stream_to_chordwise(midi_stream, chamber, note_range, note_offsets[note_range],
                                                   sample_freq)
...
def main(chamber, composers, replace):
    ...
    for i in range(len(IN_PATH)):
        ...
        for j in range(len(files)):
            ...
            translate_piece(fname, output_folder, chamber, sample_freqs, note_ranges, note_offsets, CHORDWISE_PATH,
                            NOTEWISE_PATH, replace)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--composers', dest="composers",
                        help="Specify composers (default is all, separate composers by comma)")
    parser.set_defaults(composers="")
    parser.add_argument('--replace', dest="replace", action="store_true",
                        help="Retranslate and replace existing files (defaults to skip)")
